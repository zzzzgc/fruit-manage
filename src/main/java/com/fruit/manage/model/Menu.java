...
public class Menu extends BaseMenu<Menu> {
    ...
    public List<Menu> getMenuListByUid(int uid) {
        String sql = "SELECT m.* FROM `a_user` u " +
                "INNER JOIN `a_user_role` ur ON u.`id` = ur.user_id " +
                "INNER JOIN `a_role` r ON ur.role_id = r.`id` " +
                "INNER JOIN `a_role_menu` rm ON rm.role_id = r.`id` " +
                "INNER JOIN `a_menu` m ON rm.menu_id = m.`id` " +
                "AND u.id = ? " +
